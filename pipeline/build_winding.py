...
# Winding's first field is Manz-style freeform ("pauly 9/4 445-10ish", "Katelyn Leake
# Friday mid"), so it reuses the Manz shift parser verbatim rather than forking it.
exec(open('manz_shift_parse_test.py').read().split("if __name__")[0])

# The form went live 2026-09-04; the two 7/30 rows on the sheet are pre-launch tests.
GO_LIVE = datetime(2026, 9, 4)
...
def norm_staffing(val):
    ...
    if 'over' in v or 'sent people home' in v or 'sent someone home' in v: return 'Overstaffed'
...
FIELD_MAP = [
    ...
    (4, 'Team player / needed to step up'),
    (5, 'Tardy / early out'),
    (6, 'Safety hazards'),
    (7, 'Xbro / non-negotiables'),
    (8, 'Closing: Verifone WiFi'),
    (9, 'Expired product'),
]
...
def build_winding(csv_path):
    ...
    for r in data:
        ...
        # pre-launch test submissions (or anything literally labeled a test) never count
        namefield = safe_get(r, 1)
        if dt < GO_LIVE or re.fullmatch(r'(hi!?\s*)?test!?\.?', namefield.strip(), re.I):
            skipped_test += 1
            continue
        di = day_index_for(dt)
        ...
        name = parse_name_winding(namefield)
        cutoff_h, cutoff_m = cutoff_for('Winding', shift, dt)

        staffing_raw = safe_get(r, 2)
        line_times = safe_get(r, 3)
        team_player = safe_get(r, 4)
        tardy_raw = safe_get(r, 5)
        safety_raw = safe_get(r, 6)
        xbro_raw = safe_get(r, 7)
        expired_raw = safe_get(r, 9)

        staffing = norm_staffing(staffing_raw)
        safety_status, safety_detail = norm_safety(safety_raw)

        if shift == 'Close':
            if cutoff_h < 5:  # after-midnight cutoff (e.g. 00:15)
                is_late = 0 <= dt.hour <= 3 and (dt.hour, dt.minute) > (cutoff_h, cutoff_m)
            elif dt.hour == cutoff_h: is_late = dt.minute > cutoff_m
        ...
        if line_times.strip():
            is_bad = bool(re.search(r'rough|slam|short staff|struggl|behind|disorganiz|heated|upset', line_times, re.I))
            if is_bad:
                flags.append({'kind':'bad','employee':None,'text':line_times.strip()})
        if is_late:
        ...
        for nm in extract_possessive(line_times): mentions.append((nm, 'good', line_times))
        for nm in extract_shoutout(line_times): mentions.append((nm, 'good', line_times))
        ...
        out.append({
            'shop': 'Winding', 'shift': shift, 'dayIndex': di, 'timestamp': ts,
            'employee': name, 'isLate': is_late,
            'bestPlayer': None, 'needsHats': None, 'tardy': None, 'hazard': None, 'xbroStruggle': None,
            'comment': line_times.strip() if line_times.strip() else 'No comments, standard shift.',
            'flags': flags, 'fullRecap': full_recap, 'namedMentions': named_mentions,
        })
    return out, skipped_test

if __name__ == '__main__':
    records, skipped_test = build_winding('winding_recap_raw.csv')
    with open('winding_records_full_window.json', 'w') as f:
        json.dump(records, f, indent=2)
    print(f'{len(records)} Winding records built ({skipped_test} pre-launch/test row(s) excluded)')
